...
6. ~~Air backend blow-out silently ignored~~ — `air_pip_backend.py` `dispense()` overrode the syringe backend but never called `_perform_blow_out()`, so `blow_out_air_volume` was silently dropped. Also fixed plunger overrun: main dispense was pushing out all air gaps (lag + tag) leaving zero travel for blow-out. Now holds back air gap volume when blow-out is requested and caps at available air. Fixed 2026-05-05.
7. ~~Aspiration zadd tracking cap~~ — hardcoded `zadd` cap of 32 units (3.2mm) in `air_pip_backend.py` prevented proper liquid level tracking for larger volumes. For 200uL in 250uL Eppendorf plate (area=33.2mm²), liquid drops ~6mm but tracking was limited to 3.2mm — tip lost contact with liquid and aspirated air. The plate's `z_max` already limits depth, making the cap redundant. Removed. Fixed 2026-05-12.
8. **Per-channel X alignment drift at depth** — channels appear aligned at home Z but show small X offsets as they descend, likely due to slightly non-vertical Z shafts. Causes tips to contact well walls instead of centering. Functional for 96-well plates (6.8mm opening) but may be a risk for 384-well (3.6mm opening). No software fix — firmware only supports a single X position for all channels. Mechanical correction (shaft straightening) required if 384-well work is needed. Observed 2026-05-06.
